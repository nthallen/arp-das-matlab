classdef data_field < handle
  ...
  methods
    function df = data_field(dfs, var_name, fmt, varargin)
      ...
      lbltxt = [opts.label ':'];
      df.lbl = uilabel('Text',opts.lbltxt, ...
        'HorizontalAlignment', 'left', ...
        'BackgroundColor', dfs.figbgcolor, ...
        'FontWeight', 'bold', ...
        'tag', df.lbl_name, ...
...
%      df.fld_height = max(df.lbl.Position(4), df.txt.Position(4));
      if ~isempty(opts.units)
        df.units = uilabel('Text',opts.units, ...
          'HorizontalAlignment', 'left', ...
          'BackgroundColor', dfs.figbgcolor, ...
          'userdata', df);
        df.units.Layout.Row = cntx.Row;
        df.units.Layout.Column = cntx.Column + 2;
      end
